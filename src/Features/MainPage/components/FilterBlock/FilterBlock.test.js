...
describe('<FilterBlockTest  />', () => {
  ...
  it('Renders FilterBlockTest component', () => {
    const tree = shallow(< FilterBlockTest {...mockProps} location={location}/>);
    expect(toJson(tree)).toMatchSnapshot();
  });
  it('Renders FilterBlockTest component snapshot2', () => {
    const tree = mount(< FilterBlockTest {...mockProps} location={location}/>);
    expect(toJson(tree)).toMatchSnapshot();
  });
  it('should call transitionToURL on init', () => {
    const tree = shallow(<FilterBlockTest
                {...mockProps}
                location={location}
            />);
  ...
  it('FilterBlockTest gets new props', () => {
    ...
    const tree = mount(<FilterBlockTest
                {...mockProps}
                sortBy={sortBy1}
  ...
  it('should call handleReleaseDateSort on click with release_date', () => {
    const tree = mount(<FilterBlockTest {...mockProps} location={location} sortBy="release_date"/>);
  ...
  it('should call handleRatingSort on click with vote_average', () => {
    const tree = mount(<FilterBlockTest {...mockProps} location={location} sortBy="vote_average"/>);
    const { setSortBy } = mockProps;
    tree.find('.b-result__sort-item_active').simulate('click');
    expect(setSortBy).toHaveBeenCalled();
  });
  it('should call handleGenresSearch on click with genres', () => {
    const tree = mount(<FilterBlockTest {...mockProps} location={location} searchBy="genres"/>);
    const { setSearchBy } = mockProps;
    tree.find('.b-header__button_active').at(1).simulate('click');
    expect(setSearchBy).toHaveBeenCalled();
  });
  it('should call handleGenresSearch on click with genres', () => {
    const tree = mount(<FilterBlockTest {...mockProps} location={location} searchBy="title"/>);
    const { setSearchBy } = mockProps;
  ...
  it('should call handleEnterPress on press enter button', () => {
    ...
    const tree = mount(<FilterBlockTest {...mockProps} location={location} history={history}/>);
    const { handleEnterPress, fetchMovies } = mockProps;
  ...
  it('should call handleSearchButton', () => {
    ...
    const tree = mount(<FilterBlockTest {...mockProps} location={location} history={history}/>);
    const { handleSearchButton, fetchMovies } = mockProps;
